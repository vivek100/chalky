  ...
  return (
    <header className="sticky top-0 z-40 border-b border-moss-line bg-white/95 backdrop-blur">
      <div className="moss-container flex h-16 items-center justify-between gap-4">
        <Link href="/integrations" className="focus-ring flex items-center gap-2 rounded-md">
        ...
        <nav className="hidden items-center gap-7 text-sm text-moss-muted md:flex">
          {nav.map((item) => (
            <Link className="focus-ring rounded-md hover:text-moss-ink" key={item.href} href={item.href}>
              {item.label}
            </Link>
          ))}
        </nav>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 bg-white p-5 md:hidden">
          ...
          <nav className="mt-8 grid gap-2 text-lg">
            {nav.map((item) => (
              <Link className="rounded-xl px-3 py-3 hover:bg-moss-soft" key={item.href} href={item.href} onClick={() => setOpen(false)}>
                {item.label}
              </Link>
            ))}
          </nav>
